...
public class HelloWorldController {
	...
	@Value("${client.hello.name.message:Hello %s from client}")
	private String HELLO_NAME_FROM_CLIENT;
	...
	public ResponseEntity<String> sayHelloWorld() {
		...
		return new ResponseEntity<String>("Message from client ->"+re.getBody(), HttpStatus.OK);
	}

	@RequestMapping(value = "/helloName/{name}", method = RequestMethod.GET)
	public ResponseEntity<String> sayHelloWorld(@PathVariable String name) {
		return new ResponseEntity<String>(format(HELLO_NAME_FROM_CLIENT, name), HttpStatus.OK);
	}
